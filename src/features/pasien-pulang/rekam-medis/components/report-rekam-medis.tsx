...
interface RekamMedisGeneralConsentProps {
  isReport?: boolean
  noreg?: string
  no?: string
}

export default function RekamMedisGeneralConsent({
  isReport = false,
  noreg = '',
  no,
}: RekamMedisGeneralConsentProps) {
  const { pasien } = usePasienStore((state) => state)

  const { data, isLoading, isError } = useQuery({
    queryKey: [constants.queryKey.rekamMedis.rekamMedis, noreg],
    queryFn: () => getGeneralConsent(noreg, pasien!.kd_bagian),
    enabled: !!noreg,
  })
  ...
  return (
    <LayoutBorderNo isReport={isReport} no={no}>
      <div className='p-4 text-xs'>
        <h3 className='text-xs font-bold text-center py-3'>
