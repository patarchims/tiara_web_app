...
import { onGetSbar } from '@/services/sbar'
...
export function SBARReport({ isReport = false, no_reg = '' }: SBARProps) {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => onGetSbar(no_reg != '' ? no_reg : pasien!.no_reg),
  })
    ...
    <LayoutBorderNo no='RM.RI ?' isReport={isReport}>
      ...
      <div className='p-2'>
        <h1 className='text-center text-black font-semibold mb-4'>
          CATATAN PERKEMBANGAN PASIEN TERINTEGRASI (Tulisan dengan format SBAR)
        </h1>
