...
export default function UpdateSbar() {
  const { formUpdateSbar, onUpdateSbar } = useSbar()
  ...
  return (
    <>
      <div className='rounded-md border border-gray-400 h-[calc(80vh)] overflow-x-auto px-4 py-2'>
        <Form {...formUpdateSbar}>
          <form
            onSubmit={formUpdateSbar.handleSubmit(onUpdateSbar)}
            className='space-y-4'
          >
            <FormField
              control={formUpdateSbar.control}
              name='situation'
              render={({ field }) => (
                <FormItem>
      ...
      <Button
        onClick={formUpdateSbar.handleSubmit(onUpdateSbar)}
